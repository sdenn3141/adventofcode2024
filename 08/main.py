        ...
        for key in self.nodes.keys():
            ...
            for pair in pairs:
                ...
                if self.harmonics_enabled:
                    new_antinodes = [
                        [pair[0][0] + y_gap * i, pair[0][1] + x_gap * i]
                        for i in range(-len(self.area), len(self.area))
                    ]
                else:
                    new_antinodes = [
                        [pair[0][0] + y_gap * 2, pair[0][1] + x_gap * 2],
                        [pair[0][0] - y_gap, pair[0][1] - x_gap],
                    ]
                for a in new_antinodes:
                    if (
                        a[0] < 0
                        or a[0] > len(self.area) - 1
                        or a[1] < 0
                        or a[1] > len(self.area[0]) - 1
                    ):
                        continue
                    else:
                        self.area[a[0]][a[1]] = "#"
